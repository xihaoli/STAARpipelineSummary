...
#' @return \code{results_ptv_genome.Rdata}: a matrix contains the STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the coding mask defined by the protein-truncating variants (ptv) for all protein-coding genes across the genome.
#' @return \code{ptv_sig.csv}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ptv masks.
#' @return \code{ptv_sig_cond.csv}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ptv masks (available if known_loci is not a NULL).
#' @return \code{results_ptv_ds_genome.Rdata}: a matrix contains the STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the coding mask defined by the protein-truncating variants and disruptive missense variants (ptv_ds) for all protein-coding genes across the genome.
#' @return \code{ptv_ds_sig.csv}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ptv_ds masks.
#' @return \code{ptv_ds_sig_cond.csv}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ptv_ds masks (available if known_loci is not a NULL).
...
Gene_Centric_Coding_Results_Summary_incl_ptv <- function(agds_dir,gene_centric_coding_jobs_num,input_path,output_path,gene_centric_results_name,
	...
	results_ptv_genome <- c()
	results_ptv_ds_genome <- c()
	...
	if(length(known_loci)!=0)
	{
		if(!use_SPA)
		{
			...
			if(length(ptv_sig)!=0)
			{
				if((class(ptv_sig)[1]!="matrix")&(class(ptv_sig)[1]!="data.frame"))
				{
					ptv_sig <- matrix(ptv_sig,nrow=1)
				}

				for(k in 1:dim(ptv_sig)[1])
				{
					chr <- as.numeric(ptv_sig[k,2])
					gene_name <- as.character(ptv_sig[k,1])
					category <- as.character(ptv_sig[k,3])
			...
			write.csv(ptv_sig_cond,file=paste0(output_path,"ptv_sig_cond.csv"))

			### ptv_ds
			ptv_ds_sig_cond <- c()
			if(length(ptv_ds_sig)!=0)
			{
				if((class(ptv_ds_sig)[1]!="matrix")&(class(ptv_ds_sig)[1]!="data.frame"))
				{
					ptv_ds_sig <- matrix(ptv_ds_sig,nrow=1)
				}

				for(k in 1:dim(ptv_ds_sig)[1])
				{
					chr <- as.numeric(ptv_ds_sig[k,2])
					gene_name <- as.character(ptv_ds_sig[k,1])
					category <- as.character(ptv_ds_sig[k,3])
			...
			write.csv(ptv_ds_sig_cond,file=paste0(output_path,"ptv_ds_sig_cond.csv"))
			...
			coding_sig_cond <- rbind(coding_sig_cond,ptv_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","SKAT(1,25)","Burden(1,1)","ACAT-V(1,25)","STAAR-O")])
			coding_sig_cond <- rbind(coding_sig_cond,ptv_ds_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","SKAT(1,25)","Burden(1,1)","ACAT-V(1,25)","STAAR-O")])
		...
		}else
		{
			...
			if(length(ptv_sig)!=0)
			{
				...
				for(k in 1:dim(ptv_sig)[1])
				{
					...
					{
						res_cond <- ptv_sig[k,,drop=FALSE]
						res_cond[1,3] <- "ptv_cond"
					}
			...
			if(length(ptv_ds_sig)!=0)
			{
				...
				for(k in 1:dim(ptv_ds_sig)[1])
				{
					...
					{
						res_cond <- ptv_ds_sig[k,,drop=FALSE]
						res_cond[1,3] <- "ptv_ds_cond"
					}
			...
			coding_sig_cond <- rbind(coding_sig_cond,ptv_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,1)","STAAR-B")])
			coding_sig_cond <- rbind(coding_sig_cond,ptv_ds_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,1)","STAAR-B")])
	...
	if(QQ_plot)
	{
		...
		legend("topleft",legend=c("pLoF","pLoF+D","Missense","Disruptive Missense","Synonymous","PTV","PTV+D"),ncol=1,bty="o",box.lwd=1,pch=0:6,cex=1,text.font=2)
